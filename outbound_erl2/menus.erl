...
->
    ...
    logger:debug(#{ caller => maps:get(caller_number, Data, "n/a"), content => CurrentContent }),
...
->
    ...
    case ContentType of
        ...
        section ->
            menu_queue(State, Data, [ q(empty_section) ])
    end;
...
prompt(PromptName) -> % {{-
    case PromptName of
        ...
        "general_announcement" ->
            "The California voter information guide for the November 8, 2022, General Election is now in category 5."
            % "Please listen to the announcement after the ad updates. "
            "Tuesday, October 11th: The latest Aldi and Walgreens ads are now available in category 1. "
            "Sunday, October 9th: The latest CVS ads are now available in category 1. "
            "Saturday, October 8th: The latest Costco ads are now available in category 1. "
            "Friday, October 7th: The latest SaveMart, Lucky Supermarkets, and Kroger ads are now available in category 1. "
            "Thursday, October 6th: The latest Safeway, Raley's, Sprouts, Pharmaca, Foods Co., La Superior Supermercados, and Rancho San Miguel Markets ads are now available in category 1. "
            "Coffee & Connect Vision Loss Support Group Teleconference Meetings on Tuesdays and Thursdays at 10:30 AM to 12 PM.  The dial-in Numberis  8,4,8; 7,7,7; 1,2,1,2, and the Conference ID is 1,2,3,8,0,0, pound. For more information, contact the Senior IMPACT Project at 9,1,6; 8,8,9; 7,5,1,6."
            ;
        ...
        "demo_announcement" ->
            ...
            % "approximately  5 minutes  to  try out  the "
            % "system before getting disconnected. "
            "Please  note   that  the   current  system "
            "identifies   listeners    by   caller   ID "
            "therefore if you are a registered user but "
            "calling from  a different number  than the "
            "one you  signed up with,  or if you  are a "
            "new  user  and  would  like  to  register, "
            "please call 9 1 6, 8 8 9, 7 5 1 9 or leave "
            "a message  by pressing  0 and  then pound. "
            "Thank you! ";
        ...
        % `empty_publication`    and    `empty_section`    are
        % empty   strings   because   it   is   handled   with
        % `article-number-*`
        "empty_publication" ->
            "";
...
% TODO 20221102_0621
% Come up with better names; the names of `menu_queue`, `queue_prompt`, and (especially) `q/1` makes it hard to fathom what they do and how they relate to each other.
...
->
    logger:debug(#{ 'menus:playback/3' => [{playbackname, PlaybackName}, {path, Path}, {data, Data}] }),
...
% TODO 20221102_0607 delete
% Only used in play/? and it is just to re-order the args of playback(Name, Data, Path) to playback(Data, Name, Path). So delete this one, and either re-order playback/3's params, or make it a map. (More verbose but at least explicit. Or make it a tuple list. Whatever.)
...
% Needed because when started playing the main menu immediately after the call was answered, the beginning of the playback was clipped at times, and this gives FreeSWITCH time to connect the audio channels and whatnot. Also, the noise generation is necessary, because otherwise there would only be silence and one could think that nothing is working or the line went dead (this is especially true when navigating between menus so it would feel like the call got disconnected).
comfort_noise(Name, Milliseconds, Data) ->
    logger:debug(#{ 'menus:comfort_noise/3' => ["play comfort noise"] }),
    ComfortNoise =
           "silence_stream://"
