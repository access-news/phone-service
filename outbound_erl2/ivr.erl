...
init(_Args) -> % {{-
    ...
    % TODO Why was this necessary? I thnk because this is not a proper OTP application, and it is run from a local erl repl; so when it crashes, it would take the whole shell with it.
    % Anyway, this process shouldn't trap EXITs, especially when it will be put into a supervision tree
...
%% -----------------------------------------------------
%% https://www.erlang.org/doc/man/gen_statem.html#type-callback_mode
%%
%% Short
%% ====================================================
...
%% STATE ENTER %% %{{-
%% =========== %%
%% This is where all playback starts and ends (or, more appropriately, vice versa) on state transition.
handle_event(enter, OldState, State, Data) ->

    % TODO 20221102_0543
    % There is sometimes (often..) a delay in processing DTMF events. That is, the menu still plays even a couple seconds after the selection has been made and does not stop playback right away.
    % + Is this simply because of lag?
    % + Or the VM should be beefed up?
    % + Maybe I'm using FreeSWITCH's event system wrong? See note in menus.erl -> "NOTE on use of  sync and async FreeSWITCH events". There may need to be some experimentation with mixing "sync" & "async" playback.

    % QUESTION (OffsetReset anon function) Does this actually do anything? {{-
    ...
    % ANSWER It does
    %
    % UPDATE 20221102_0529
    % Don't quite understand my original question (and the answer) as this clause definitely does what it needs to do, but the question may refers to the OffsetReset anonymous function below. Anyway, not deleting the question yet as it may point to a bug.

    logger:debug(#{ 'ivr:handle_event(enter, ...)' => [ {from, OldState}, {to, State}, {data, Data} ]}),
    ...
    NewData =
        futil:pipe(
          ...
          % time.
          % TODO 20221102_0531 This should be `stop_all_playback` to make this clear (even though I do not think that multiple playbacks would be possible anyway)
          % UPDATE on (TODO 20221102_0531) Uh-oh, this is more nuanced than that: see notes in menus.erl -> "NOTE on use of  sync and async FreeSWITCH events"
...
%% , and the fastest way to parse that is using `proplists` function, and it cannot be matched in function headers.
%% }}- }}-
...
% 20221028_1129 TODO UUID, uuid is called CallUUID in fs.erl. Make this consistent.
handle_event
( internal                           % EventType
, { UUID                             % \
  , call                             % |
...
->
    % 20221028_1143 QUESTION how do we know that this is an INCOMING_CALL when this is not even mentioned in the function header? This is one of those undocumented FS events vs mod_erlang_event discrepancies...
    % 20221028_1146 ANSWER This seems to answered in the comment preceding the function header (titled "Call init") but this should be made explicit
...
->
    logger:debug(#{ event => TempEvent, data => Data, state => incoming_call }),
    {next_state, greeting, Data};

%% }}-

%% (STATE: * -> *) - catch-all state-handler!
...
handle_event(
  ...
  , #{ "Event-Name" := "DTMF" % | EventContent = MassagedModErlEvent
     , "DTMF-Digit" := Digit  % | TODO TEST THIS!  NOTE 20221028_1141 Test what?
     }                        % |
  },                          % /
  State,
    ...
    case State of
        ...
        content_root ->
            case Digit of
                ...
                % }}-
                % [1-9]      {{- => collect digits
                % 20221024_1619 TODO superfluous to check: `collect_digits` does it already, so it will crash if anything other than digits come through (which, in fact, can happen; there are some very old phone with A, B, C, D DTFM signals that have crashed TR2 before...)
                _ when Digit =:= "1"
                     ; Digit =:= "2"
                     ; Digit =:= "3"
                     ; Digit =:= "4"
...
->
    logger:emergency(#{ self() => ["UNKNOWN_INTERNAL_CALL_EVENT", #{ event_name => EventName, state => State, fs_event => FSEvent}]}),
    keep_state_and_data;
...
% 20221024_1611 QUESTION Shouldn't this crash? Couldn't find a clause that would handle this scenario...
% 20221024_1616 ANSWER As far as I could tell, this clause will never get evaluated because * and # are checked in the state machine itself above so `collect_digits` will never get called with * and #.
% TODO on that note: This clause is superfluous (and incorrect, as there is no such clause)
collect_digits(Digit, Data)
    when Digit =:= "*"
       ; Digit =:= "#"
->
...
% 20221028_1128 TODO: This should be a convenience function in `user_db`
...
% TODO PROD test this (with corresponding handle clause); amended after the re-write, but no clue if it will blow up or not
    ...
    Timeouts = % in minutes
        case State of
            ...
            _ ->
                #{ warning => 90 % changed from 10 (it would hang up mid-recording)
                 , timeout => 100
                 }
        end,
