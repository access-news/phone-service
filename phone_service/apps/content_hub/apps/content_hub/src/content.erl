...
% ( [ first
  , #{} = ContentItem
  % , { { ContentType, Title, Meta } = ContentItem
  %     , SubItems
  %   }
  % | RestContentItemWithSubItems
  % ]
, Graph
, ParentVertex
)
...
when is_list(SubItems), is_list(RestContentItemWithSubItems)
->
    DrawFunction =
        get_draw_function(ContentItem),

    case { PrevItemVertex, RestContentItemWithSubItems } of
        { first, [] } ->
...
->
    ...
    LeafItemVertex =
        to_vertex_data(LeafContentItem, ItemNumber),
...
->
    ...
    ItemVertex =
        to_vertex_data(ContentItemWithSubItems, ItemNumber),
...
%     % erlang:display([draw_item, #{ rest => RestContentItems}]),
    ...
    case Direction of
        ...
        % last ->
        %     add_edge(Graph, last, ParentVertexData, VertexDataB);
...
, ItemNumber
    ...
    futil:pipe(
      [ ContentItemHeader
      , (futil:curry(fun maps:merge/2))(Add)
